...
export const doAddComment = (comment, commentId, articleId) =>
  db.ref('/comments/' + articleId + '/' + commentId).update(comment)

export const doEditArticle = (articleId, { title, body, lastEdit }) =>
  db.ref('/articles/' + articleId).update({ title, body, lastEdit })

export const doEditUserInfo = (userId, name, surname, age, about) =>
  db.ref('/users/'+userId).update({ name, surname, age, about })

export const doEditUserEmail = (userId, newEmail) =>
  db.ref('/users/'+userId).update({ email: newEmail })

export const doUpdateUserPhoto = (userId, photoUrl) =>
  db.ref('/users/'+userId).update({ photoUrl })
